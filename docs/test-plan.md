...
```powershell
cd D:\AI\AdobePlugin\AdobeExpress\social-promo-asset-rollout-checker
npm test
npm run validate
npm run build
```
...
- `npm run validate` で `dist/social-promo-asset-rollout-checker-representative-report.json` を生成する。
